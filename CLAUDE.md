...
- **Claude can build and test the whole sim layer headlessly.** The sim core is pure Rust with zero rendering dependencies, so `cargo build`, `cargo test`, `cargo clippy`, and `cargo fmt` all run without a front end or a windowing system. This is the primary verification loop — use it freely after touching sim/rules code, and prefer it to prose for pinning behavior. **In Claude Code on the web/mobile**, the Rust toolchain is provisioned by an async `SessionStart` hook, so before the *first* `cargo` command in a remote session run `.claude/hooks/wait-for-rust.sh` (blocks until ready; no-op once cargo is available, and entirely unnecessary on the user's local machine where rustup is already installed).
- **Front-end behavior — Claude can now *see* it via Playwright.** `e2e/` drives the web UI in a headless browser and writes screenshots to `e2e/shots/`; run `npm test` in `e2e/` (with a `tithe-web` dev server up, or it starts one), then **Read the PNGs** to verify the editor and the live match render. Use this after touching front-end code instead of guessing. Limits: it's a snapshot, not full interaction-feel or device input, and Tauri/desktop still needs the user — so for nuanced *feel* (animation, responsiveness), still name what to verify and have the user run it. (The watch view is fundamentally *validated by eye* — §9; Playwright just gives Claude eyes for the common cases.) Remote (Claude Code web): first run `npm ci && npx playwright install chromium` in `e2e/` (Node assumed present; the Rust toolchain comes from the session-start hook).
